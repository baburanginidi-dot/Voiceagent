...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onExit }) => {
  ...
  // Generic API wrapper with auth + 401 handling
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Expect config as array of { id, key, value }
      const systemPrompt =
        configData.find((c: any) => c.key === 'systemPrompt')?.value || '';
      const stagesData =
        configData.find((c: any) => c.key === 'stages')?.value || [];
      ...
      // Basic analytics derived from logs for now
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      const newDoc: StageDocument = {
        id: Date.now().toString(), // backend will replace with UUID
        name: file.name,
        content: content,
  ...
  const renderSidebar = () => (
    <div className="w-64 bg-white border-r border-[#EAEAF0] h-full flex flex-col">
      ...
      <nav className="flex-1 p-4 space-y-2">
        {(['dashboard', 'prompts', 'stages', 'logs'] as Tab[]).map((tab) => (
          <button
            ...
            className={`w-full text-left px-4 py-3 rounded-[12px] text-sm font-medium transition-colors capitalize ${
              activeTab === tab
                ? 'bg-black text-white'
      ...
      <div className="p-4 border-t border-[#EAEAF0]">
        <button
          onClick={onExit}
          className="w-full px-4 py-2 border border-[#EAEAF0] rounded-[12px] text-sm font-medium text-[#FF3B30] hover:bg-[#FFF5F5]"
        >
          Exit to App
        </button>
      </div>
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[24px] border border-[#EAEAF0] shadow-sm">
          <div className="text-[#8E8E93] text-sm font-medium mb-1">
            Total Calls
          </div>
  ...
  const renderPrompts = () => (
    <div className="space-y-4 h-full flex flex-col animate-fadeIn">
      ...
      <p className="text-[#4F4F4F] text-sm">
        This is the core personality and logic driver. Variables like{' '}
        <code className="bg-[#F2F2F2] px-1 rounded">
            ...
            <div
              ...
              className={`bg-white rounded-[20px] border transition-all duration-300 overflow-hidden ${
                isExpanded
                  ...
                  : 'border-[#EAEAF0] hover:border-[#D1D1D6]'
              }`}
            >
              <div
              ...
              {isExpanded && data && (
                <div className="px-5 pb-6 pt-2 bg-[#F9F9FB]/50 border-t border-[#EAEAF0] space-y-5 animate-fadeIn">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-xs font-bold text-[#4F4F4F] uppercase tracking-wider mb-2">
                        System Prompt
                      </label>
                      <textarea
                        value={data.systemPrompt || ''}
                        onChange={(e) =>
                          handleStageChange('systemPrompt', e.target.value)
                        }
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          handleStageChange('knowledgeBase', e.target.value)
                        }
                  ...
                  <div className="pt-2">
                    <label className="block text-xs font-bold text-[#4F4F4F] uppercase tracking-wider mb-3">
                      ...
                      {data.documents && data.documents.length > 0 && (
                        <div className="space-y-2 mb-4">
                          {data.documents.map((doc) => (
                            <div
                              key={doc.id}
                              className="flex items-center justify-between bg-[#F9F9FB] p-3 rounded-[12px] group"
                            >
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 bg-black/5 rounded-lg flex items-center justify-center text-lg">
                                  📄
                                </div>
                                <div>
                                  ...
                                  <div className="text-[10px] text-[#8E8E93] uppercase">
                                    {doc.type || 'text/plain'}
                                  </div>
                                </div>
                              </div>
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-3">
            <div className="relative">
              ...
              />
              <svg
                className="w-4 h-4 text-[#8E8E93] absolute left-3.5 top-1/2 -translate-y-1/2"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
        ...
        <div className="bg-white rounded-[24px] border border-[#EAEAF0] overflow-hidden shadow-sm">
          <table className="w-full table-fixed">
            <thead className="bg-[#F9F9FB] border-b border-[#EAEAF0]">
              <tr>
                <th className="w-32 px-6 py-4 text-left text-xs font-semibold text-[#8E8E93] uppercase tracking-wider">
                  Student
                </th>
                <th className="w-32 px-6 py-4 text-left text-xs font-semibold text-[#8E8E93] uppercase tracking-wider">
                  Phone
                </th>
                <th className="w-24 px-6 py-4 text-left text-xs font-semibold text-[#8E8E93] uppercase tracking-wider">
                  Date
                </th>
                <th className="w-32 px-6 py-4 text-left text-xs font-semibold text-[#8E8E93] uppercase tracking-wider">
                  Status
                ...
                <th className="w-24 px-6 py-4 text-right text-xs font-semibold text-[#8E8E93] uppercase tracking-wider">
                  Action
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#EAEAF0]">
              {filteredLogs.map((log) => (
                ...
                >
                  <td className="px-6 py-4 text-sm font-bold truncate">
                    {log.studentName}
                  </td>
                  <td className="px-6 py-4 text-sm text-[#4F4F4F] font-mono truncate">
                    {log.phoneNumber}
                  </td>
                  <td className="px-6 py-4 text-sm text-[#4F4F4F] truncate">
                    {log.date?.split(' ')[0]}
                  </td>
            ...
            <div className="bg-white rounded-[24px] w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
              <div className="p-6 border-b border-[#EAEAF0] flex justify-between items-start bg-[#F9F9FB]">
                ...
                <button
                  ...
                  className="p-2 hover:bg-[#EAEAF0] rounded-full transition-colors"
                >
                  <svg
                  ...
                  <div className="space-y-3">
                    {selectedLog.transcript?.map((t) => (
                      ...
                      >
                        ...
                        >
                          <div className="text-[10px] opacity-50 mb-1 uppercase tracking-wide">
                            {t.sender}
                          </div>
                          {t.text}
                        </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F5F5F7] font-['Inter'] p-6">
        <div className="w-full max-w-[360px] bg-white p-8 rounded-[32px] shadow-lg border border-[#EAEAF0]">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              <label className="block text-xs font-semibold text-[#4F4F4F] mb-1.5 ml-1">
                USERNAME
              </label>
              <input
                type="text"
